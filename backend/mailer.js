...
// ── Brevo SMTP (works on Render free tier — Gmail SMTP is blocked by Render)
...
//   BREVO_PASS = your brevo SMTP key (starts with xsmtp...)
...
const base = `font-family:'Segoe UI',Arial,sans-serif;background:#0a0e1a;color:#e8edf5;padding:0;margin:0;`;
const card = `max-width:520px;margin:32px auto;background:#111827;border-radius:16px;overflow:hidden;border:1px solid #1e2d45;`;
const hdr  = c => `background:${c};padding:28px 32px;text-align:center;`;
...
async function sendWelcome({ name, email, roll }) {
  ...
  await transporter.sendMail({
    ...
    html: `<body style="${base}"><div style="${card}">
      <div style="${hdr('#1a2d4a')}">
        <h1 style="color:#4d9fff;margin:0;font-size:28px;">📚 LibraSense</h1>
        <p style="color:#6b7a99;margin:8px 0 0;font-size:14px;">SRM KTR Library Seat Management</p>
      </div>
      <div style="padding:32px;">
        <h2 style="color:#e8edf5;margin:0 0 8px;">Welcome, ${name}! 👋</h2>
        <p style="color:#6b7a99;line-height:1.7;">Your account is ready. Book and check in to library seats across Ground Floor, Level 1 and Level 2 in real time.</p>
  ...
  await transporter.sendMail({
    ...
    html: `<body style="${base}"><div style="${card}">
      ...
      <div style="padding:32px;">
        ...
        <p style="color:#6b7a99;font-size:13px;margin-top:20px;">⚠️ Seat auto-releases if you don't check in before time expires.</p>
      </div>
    </div></body>`,
  });
}
...
async function sendExpiry({ name, email, seatId }) {
  ...
  await transporter.sendMail({
    ...
    html: `<body style="${base}"><div style="${card}">
      <div style="${hdr('#2a1a0f')}">
        <p style="color:#ffd166;margin:0;font-size:36px;">⏰</p>
        <h2 style="color:#ffd166;margin:8px 0 0;">Reservation Expired</h2>
      </div>
      <div style="padding:32px;">
        <p style="color:#6b7a99;">Hi ${name}, your reservation for <strong style="color:#e8edf5;font-family:monospace;">${seatId}</strong> expired and was released.</p>
        <p style="color:#6b7a99;font-size:13px;">Book a new seat any time from the LibraSense portal.</p>
      </div>
    </div></body>`,
  });
}
